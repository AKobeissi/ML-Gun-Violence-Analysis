...
summary(data)
```
Data has starting shape of (239677, 29)

Important features:
- Data contains time-series component "date" ("Year/month/date"), useful for temporal analysis and time-series. Starting date is 2013-01-01, ending date is 2018-03-31

- Geographical and geospatial data: "state", "city_or_county", "address", "latitude", "longitude", "locaiton_description"

- Political/geopolitical data: "congressional_distrect", "state_house_district", "state_senate_district"

- Demographic/personal data: "participant_age", "participant_age_group", "participant_gender", "participant_name", "participant_relationship", "participant_status", "participant_type"

- Quantitative descriptors of the gun violence incident: "n_killed" (max 50), "n_injured" (max 53), "n_guns_involved" (max 400)

- Qualitative descriptors of the incident: "gun_stolen", "gun_type", "incident_characteristics", "notes"

Useless Features: "incident_url", "source_url", "incident_url_fields_missing", "sources". We can drop these features for our analysis. Could be useful for more in depth research and insights but not for this purporse.

This, we can split up our exploratory analysis into trends based on these data groups; time related trends, location/political related trends, and demographic trends, all compared to the "n_killed" and "n_injured" features. Also, by having a simplifying assumption that each observation is independant, we can count an gun related incident as the number or obserations in our data set. We can also 
...
From the chart as well as percent_missing column, we can see that there is significant observations missing for "participant_relationship", "location_description", and "participant_name". So we can immediatley drop these columns form our analysis as they have too many missing values to consider imputation. 

```{r}
data = subset(data, select = -c(participant_relationship, location_description, participant_name))
```
Now, for "gun_type" and "gun_stolen" we can look at imputation techniques. Filling the NA cells with unkown would be a viable option, as this is already done with a lot of observations.


### Evaluating Distributions and Relationships 
```{r}
sapply(data, class)
...
plot_vicitms
```
Note: Interesting to see 400 has n_guns_involved. Upon examination, this only occured 4 times, and happened due to a raid and "buy backs", where the police confiscated guns. We will continue investigating.

It seems like the number of people killed and injured is genereally low, with the majority of observations lying between 0 to 4, however there are some cases where the incedints are violent.

### Number of incidents time-series
...
```
As can be seen from the graph, there is a general trend of an increase in gun related incidents in the USA over 2014 to 2017. Our data only contains the first quarter of 2018, however, based on previous years, it is likely that 2018 will exceed previous years. We can also see that in general the first quarter has the least incidents, whereas the middle quarters tend to increase. 

Now, evaluating the geographical features with the number of incidents.

Lets now look at the relative incidents compared to state populaitons.
```{r}
library(dplyr)
...
Total to violence ratio per 100k per year


Now, combining everything into the original data.
```{r}
...
From this graph, we can see that Illinois, Arizona, Alabama, Nevada, and Missouri are the most dangerous states in terms of violence and human reprocussions. Although from the map below, we clearly see that california, illinois, florida, and texas are the ones with the most general incidents. 

Illinois is a state to pay close attention to since it rated highest in terms of violence ratio, as well as total incidents. 
```{r}
...
First, we are creating three new columns called average particiapnt age, average suspect age, and average victim age. 
...
Ok great! Now we have 3 more continuous numeric variable, avg suspect age, avg victim age and avg pariticpant age. Let try to attack the male and female vairbale now.
...
Now lets take a look at the district data
```{r}
distric_df <- data %>%
...
Lets do a similar variable for cities. We will try to extract data from top  cities.
```{r}
data$city_freq <- table(data$city_or_county)[data$city_or_county]
...
Now lets see what we can find in the incident reports columns.
```{r}
...
```
As can be seen from these sample outputs, it contains a lot of relevant information. It seems as if there are 672 unique categories that are used in the reports.
...
```
Lets create dummy variables, Drug/Gang/DriveBy, School Related, Definsve Use, Officer, Possession,Home/House, Stolen,Mass shooting, Child
...
Finally, lets convert state so that we can use it as a predictor.

Lets also try to make a new feature called president, so that if its trump its 1 and obama its 0. As well as stte, making it  for republican and 0 for democrat. 
```{r}
...
### Model Building
- Lets start by seeing which predictors are good.
```{r}
attach(data)
...
since our model is predicting the total to violence ratio, we should be removing variables that could potenitally have lookahead bias.The event that we are predicting is how violent an incident will be. Data that would be availabe before hand is the population, the state, gun used, if it was stolen, the year, the day, the month..., the political affiliations, 
...
```
From our second pca plot, it seems like state_house_district and state_senate_district are highly correlated. Similar for total and violent incidents (since total incidents encompasses violent incidents, lets try rmeoving violent incidents)
```{r}
...
```
It seems like state and city are important
remove defesnive, and not_stolen
```{r}
...
```
Frm here, it seems like incidents that are drug related tend to have more stolen guns, possession, use handguns and mm9 as weapons, as well as gang related 
...
```
Top house and top senate district seem to be correlated, lets remove one of those. mm9 and hanggun as well. 
```{r}
...
The vectors fo rtop_state_bonary is very short, indicating that it does not carry much weight
```{r}
...
```
Top congressional district vector is shorter an extremly close to top_house and top_senate
```{r}
...
Non violent incidents (ratio less than 0.50) tend to not occur in cities such as x y z from top city, as well as top senate district, and not gang related.From this PCA, I would choose my top binary variables as top_city, top senate, gang, possession, drive by, shooting, stolen, and drug. Also, possession and drug related vectors are orthogonal to the total_to_violent_ratio, so are not very useful
...
```
Form here, I would say important continuous variables are state and city frequency, males_count, year, avg_age

Final PCA variables (quantitative): state_freq, city_freq, Males_count, year, avg_age, top_city, top senate, gang, possession, drive by, shooting, stolen, and drug
...
```
From pca2, it seems like variables that have a high relation and explain the variance are total_victims, top_city_binary, top_congressional_district, total_incidents_per_100k, violent_incidents_per_100k, total_to_violent_ratio, Males_count, shooting_related, top_state_district, congressional_district, population, possession_related, state_senate_house, stolen_related.
...
print(plot)
```
Form the clustering, we can see that younger age groups are the most violent.
```{r}
km.4
```
RANDOM FOREST 

state_freq, city_freq, Males_count, year, avg__suspect_age, top_city_binary, top senate, gang, possession, drive by, shooting, stolen, and drug. Qualitative variables: Top_city, Top_state
```{r}
...
summary(mytree)
```
From this decision tree, we can tell that state_frequency, top_state, top_city are the top three important variables. 

top_state top_city year shooting_related top_congressional_district possession_related gang_related     drug_related

Lets try to find the optimal CP
```{r}
...
[1] shooting_related top_city         top_state        year    

Variable importance
                 top_state                   top_city                       year 
                        54                         26                          8 
          shooting_related top_congressional_district         possession_related 
                         5                          4                          1 
              gang_related               drug_related 
                         1                          1 
The oabove tree ahs the best out of sample predictive performance!
```{r}
...
```
Variable importance
  shooting_related        Males_count possession_related       drug_related             stolen 
                48                 27                  9                  5                  4 
   n_guns_involved     handgun_binary    avg_suspect_age        age_cluster 
                 3                  2                  2                  1 
...
top_state                  65.07027   1350.941014
top_city                   37.42813    554.685426
year                       56.21097    476.638900
shooting_related           43.11579    135.922281
top_congressional_district 20.14036     22.489663
possession_related         24.26034     32.154738
gang_related               30.44050     11.676352
drug_related               14.46165     15.724495
age_cluster                23.96122     21.230576
stolen                     20.40308      8.372399
handgun_binary             17.72155     13.832048

From the randomforest varImpPlot and the importance information, the most relevant predictor in terms of determining how violent incedents will be is if a state is in the state. The city, year and shooting incidents are also important factors in determining the violence.

When predicting to the number of victims, shooting related is the most important followed by Males count. When looking at the violence ratio, it seems like top state, then top city, year, and shooting realted are the most important.  So the top 5 predictors so far are top state, top city, shooting related, males count. From the random Forest, var imp plot, it seems like adding gang related and age cluster could be valuvle. 

```{r}
myforest #44.28 R squared MSR = 0.01522
```
Since non of the age values worked, lets try using the age cluster as an input to our model. 

### Testing Modles
Our goal is predicting a continous varibale, so we have a regression task. Possible models include: Linear, Quadratic, Spline, Decision Trees, RandomForest, Gradient Boosted Machines.

Lets use a linear regression as our base model, and then continue to evaluate more complex models
...
Testing for outliers, heteroskedasticity, non-linearlity
```{r}
library(car)
outlierTest(reg1)
...
```
Now lets check for non-linearlity
```{r}
require(psych)
...
```
Testing for polynomial regression for vairiables: Males_count, Year, 
```{r}
...
Now that we have tested our base regression models and explored the relationship of vairbales, lets test more complex models starting with decision trees.
...
```
On a 70, 30 test split, the first decision tree has an MSE of 0.0187
...
```
Top 7 predictors are age_cluster,Males_count, possession_related, top_city, top_congressional_district, top_state and year.


Now lets try a randomforest model with these variables. 
```{r}
...
As can be seen, the optimal number of trees is 200. The MSE at 200 trees is 0.01548, with an R^2 of 56.68%
Initial GBM Tests:
...
### Running Train Test Splits on all models to get MSE and evaluate best model for predicitve performance
Linear Regression
```{r}
...
Computing MSE on 70-30 Train Test Split for GBM
```{r}
...
K-Fold Models And GBM Interpretations:
```{r}
library(gbm)
...
With 2 components, we can detect with 30% accuracy and with 8 components we can detect with 80% accruact.
